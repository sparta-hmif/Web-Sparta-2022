...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      kasuh?.kuota !== undefined &&
      kasuh?.pendaftarSekarang !== undefined &&
      kasuh?.kuota !== null &&
      kasuh?.pendaftarSekarang !== null &&
      kasuh?.kuota >= kasuh?.pendaftarSekarang
    ) {
      return NextResponse.json(
    ...
    await prisma.userKasuh.update({
      ...
      data: {
        pendaftarSekarang: kasuh?.pendaftarSekarang
          ? kasuh?.pendaftarSekarang + 1
          : 1,
      },
    });
