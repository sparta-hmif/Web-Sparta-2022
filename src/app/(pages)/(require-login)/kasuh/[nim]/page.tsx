...
const Page = ({ params }: { params: { nim: string } }) => {
  ...
  const isAlreadyChoose = dataMyKasuh && dataMyKasuh.UserDesuh?.PendaftaranKasuh?.find((val: any) => val.kasuh.user.nim === nimKasuh) !== undefined;

  const idPendaftaranKasuh = dataMyKasuh && dataMyKasuh.UserDesuh?.PendaftaranKasuh?.find((val: any) => val.kasuh.user.nim === nimKasuh)?.id;

  const alasan = dataMyKasuh && dataMyKasuh.UserDesuh?.PendaftaranKasuh?.find((val: any) => val.kasuh.user.nim === nimKasuh)?.alasan;
      ...
      <div className="container bg-white border-4 rounded-2xl border-primaryDark-400 mx-auto w-11/12 max-w-[65rem] px-5 md:px-14 py-10">
        ...
        <div className="w-full flex flex-col justify-between items-center md:items-start md:flex-row gap-4 md:gap-11">
          <div className="w-2/3 aspect-square max-w-[20rem] md:aspect-[3/4] bg-[#D9D9D9] rounded-xl">
            <Image src={data?.imageURL ? data.imageURL : "/images/landing/sparta.png"} alt="Profile Kasuh" width={200} height={200} className="w-full h-full object-cover object-center" />
          </div>
          <div className="w-full md:flex-1 flex flex-col gap-1">
            ...
            <h6 className="text-secondaryDark-500 text-xl md:text-2xl font-koulen">
              JURUSAN: {data && (data.nim.substring(0, 3) === "182" ? "STI" : "IF")}
            </h6>
