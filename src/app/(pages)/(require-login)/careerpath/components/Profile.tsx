...
import SpartanImage from "@/../public/images/landing/sparta.png";
    ...
    <div className="rounded-xl mx-[1.5px] px-3 lg:pr-5 bg-white">
      <div>
        <div className="flex lg:gap-x-5 mt-[2px] lg:mb-10 pt-2 md:pb-2 justify-between items-center">
          <div className="w-10 h-10 md:w-[60px] md:h-[60px] lg:w-[120px] lg:h-[120px] rounded-full text-center relative z-10 overflow-hidden bg-primaryDark-400 p-4 md:m-2 lg:p-8">
            <Image
              src={currPerson?.imageURL || SpartanImage}
              fill={true}
              alt="Spartan"
            />
          </div>
        ...
        >
          <div className="text-primaryDark-400 font-sen mt-2 pb-3 md:pb-4 text-[14px] md:text-[15px]">
            Kamu berada pada posisi {currPerson?.rank} dari leaderboard,
            sehingga kamu baru bisa mengisi career path-mu pada{" "}
            {targetDate.getDate()}-
        ...
        {screenSize < 1024 ? (
          ...
          >
            <div>
              <AdditionalProfile
                scoreboard={dataPerson}
                currPersonIndex={currPersonIndex}
              />
            </div>
          </CSSTransition>
        ) : (
