...
import AttachmentList from "@/app/(pages)/(require-login)/assignment/[id]/components/AttachmentList";
import { AttachmentProps } from "@/components/LinkAttachment";
import { formatDate } from "@/app/(pages)/(require-login)/assignment/components/Preview";
...
const SubjectDetail = ({ params }: { params: { id: string } }) => {
  const { data, error, isLoading } = useSwr(
    `${process.env.NEXT_PUBLIC_WEB_URL}/api/materi/${params.id}`,
    fetcher
  );
